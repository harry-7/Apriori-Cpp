    ...
    for (; i < len && j < len;) {
        ...
        for (int ind1 = i; ind1 < j; ind1++) {
            for (int ind2 = ind1 + 1; ind2 < j; ind2++) {
                ...
                /* prune out the set if any immediate subset is in frequent */
                vector<int> tmp;
                bool frequent = true;
                for (int l = 0; l < k - 1; l++) {
                    for (int m = 0; m < k - 1; m++) {
                ...
                if (frequent) {
                    //immediate subset is not frequent so prune this
                    gen.push_back(temp);
                }
                temp.pop_back();
                temp.pop_back();
...
int main() {
    ...
    // remove the infrequent items from the table.
    purgeTable();

    /*----------- Generate two itemsets -----------*/

    // Generating triangular array
    ...
    // Check for frequency of two item sets
    ...
    // Run apriori for remaining sizes using tree
    ...
    while (freq_item_sets[current - 1].size() > 1) {
        ...
        // Iterate through all records and check if that record contains the itemset
